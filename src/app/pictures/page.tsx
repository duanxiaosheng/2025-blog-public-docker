...
export default function Page() {
	...
	const handleUploadSubmit = ({ images, description }: { images: ImageItem[]; description: string }) => {
		...
		const newPicture: Picture = { id, uploadedAt: now, description: desc, images: imageUrls }
	...
	const handleDeleteSingleImage = (pictureId: string, imageIndex: number | 'single') => {
		...
		setImageItems(prev => {
			...
			if (imageIndex === 'single') {
				for (const key of next.keys()) if (key.startsWith(`${pictureId}::`)) next.delete(key)
			} else {
				...
				for (const key of next.keys()) {
					if (key.startsWith(`${pictureId}::`)) {
						...
						if (!isNaN(oldIndex) && oldIndex > imageIndex) keysToUpdate.push({ oldKey: key, newKey: `${pictureId}::${oldIndex - 1}` })
					}
				}
	...
	const handleDeleteGroup = (picture: Picture) => {
		...
		setImageItems(prev => {
			const next = new Map(prev)
			for (const key of next.keys()) if (key.startsWith(`${picture.id}::`)) next.delete(key)
			return next
		})
	}
	...
	return (
		<>
			...
			<motion.div initial={{ opacity: 0, scale: 0.6 }} animate={{ opacity: 1, scale: 1 }} className='absolute top-4 right-6 flex gap-3 max-sm:hidden'>
				...
				{isEditMode ? (
					<>
						<motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => router.push('/image-toolbox')} className='rounded-xl border bg-blue-50 px-4 py-2 text-sm text-blue-700'>压缩工具</motion.button>
						...
						<motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setIsUploadDialogOpen(true)} className='rounded-xl border bg-white/60 px-6 py-2 text-sm'>上传</motion.button>
