...
function normalizeEditorAssetUrl(url: string | null) {
	if (!url) return url
...
export function CoverSection({ delay = 0 }: CoverSectionProps) {
	const { images, setCover, cover, addFiles, addCoverFiles } = useWriteStore()
	const fileInputRef = useRef<HTMLInputElement>(null)

	const coverPreviewUrl = useMemo(() => (cover ? normalizeEditorAssetUrl(cover.type === 'url' ? cover.url : cover.previewUrl) : null), [cover])
	...
	const handleCoverDrop = async (e: React.DragEvent<HTMLDivElement>) => {
		...
		if (files && files.length > 0) {
			...
			const resultImages = await addCoverFiles(imageFiles as unknown as FileList)
			if (resultImages && resultImages.length > 0) {
				setCover(resultImages[resultImages.length - 1])
				toast.success('已设置封面')
			}
			return
		}
	...
	const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		const resultImages = await addCoverFiles(files)
		if (resultImages && resultImages.length > 0) {
			setCover(resultImages[resultImages.length - 1])
			toast.success('已设置封面')
		}
